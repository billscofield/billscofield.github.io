...
* git mv [旧名] [新名]
* git rm [file] **Remove files from the working tree(tracked files in working tree) and from the index**

* git rm --cached [file] 删除暂存区或分支上的文件, 但本地又需要使用, 只是不希望这个文件被版本控制,然后在.gitignore文件里添加上该文件，重新提交即可 

* git rm -r [目录]  (用于递归删除目录recursive,一般要配合-f)
...
* git commit -m [message] 这里不写文件说明是所有暂存区中的文件及目录
* git commit -am [message]
* git commit -v [文件1] [文件2] [文件3] [...] (!并不提交,提交时显示所有diff信息)
...
* git checkout origin/master //切换到远程主机的master分支
